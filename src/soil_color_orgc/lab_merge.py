...
# All fields owned by the *current* image-processing run. A previous enriched
# workbook may contain historical copies of these fields; they must never win
# over current predictions when laboratory workbooks are consolidated.
...
def _strip_previous_image_results(lab: pd.DataFrame) -> pd.DataFrame:
    """Discard historical image results but preserve every laboratory field.

    This also handles workbooks previously generated by the old merger, which
    placed current colour values in e.g. L_pred while retaining stale L.
    Unknown user-supplied laboratory/metadata columns remain untouched.
    """
    stale_names = set(IMAGE_PREDICTION_FIELDS)
    stale_names.update(
        f"{field}{suffix}"
        for field in IMAGE_PREDICTION_FIELDS
        for suffix in ("_pred", "_x", "_y")
    )
    stale_columns = [col for col in lab.columns if col in stale_names]
    if stale_columns:
        print(
            "Removing historical image measurements from lab input "
            f"({len(stale_columns)} columns): {stale_columns}"
        )
        return lab.drop(columns=stale_columns)
    return lab
...
def enrich_lab_file(
    ...
    fail_on_duplicate_lab_codes:
        Kept for compatibility. Duplicate lab codes are always consolidated
        column by column; conflicts and their chosen values are reported.

    fail_on_duplicate_image_codes:
        Stop on duplicate image codes (default). If False, choose one record
        per code, preferring successful and grey-reference image records.
    """
    ...
    # Keep source statistics before resolving duplicated lab rows. Reading a
    # previously enriched workbook is supported, but old image measurements
    # must not influence the lab resolver or shadow the current predictions.
    raw_lab_row_count = len(lab)
    if {"_lab_source_file", "_lab_source_sheet"}.issubset(lab.columns):
        laboratory_sources = (
            lab.groupby(
                ["_lab_source_file", "_lab_source_sheet"],
                dropna=False,
            )
            .size()
            .reset_index(name="row_count")
        )
        lab_source_count = lab["_lab_source_file"].nunique()
    else:
        laboratory_sources = pd.DataFrame()
        lab_source_count = 1

    lab = _strip_previous_image_results(lab)
    ...
    # ------------------------------------------------------------------
    # Check the resolved laboratory codes and detect image duplicates.
    #
    # A duplicate in both tables could produce a many-to-many merge and
    # artificially multiply the number of training samples.
    # ------------------------------------------------------------------
    # Duplicate *input* lab rows have already been consolidated. Preserve
    # their original records for the audit sheets rather than overwriting
    # the resolver's report with an always-empty post-resolution DataFrame.
    if lab.loc[lab["SampleCode"].notna(), "SampleCode"].duplicated().any():
        raise RuntimeError("Duplicate SampleCodes remain after lab resolution.")
    ...
    if not fail_on_duplicate_image_codes and not duplicate_image_codes.empty:
        # Consistent with run_all.py: favour a successful record from the
        # grey-reference source, retaining one prediction per sample code.
        predictions = predictions.copy()
        predictions["_merge_gray_priority"] = (
            predictions.get(
                "image_source", pd.Series("", index=predictions.index)
            ).astype("string").str.lower().eq("with_gray").fillna(False)
        )
        predictions["_merge_ok_priority"] = (
            predictions.get(
                "processing_status", pd.Series("", index=predictions.index)
            ).astype("string").str.lower().eq("ok").fillna(False)
        )
        predictions = predictions.sort_values(
            ["_merge_gray_priority", "_merge_ok_priority"],
            ascending=False,
            kind="stable",
        )
        known_prediction_rows = predictions.loc[
            predictions["SampleCode"].notna()
        ].drop_duplicates("SampleCode", keep="first")
        unknown_prediction_rows = predictions.loc[
            predictions["SampleCode"].isna()
        ]
        predictions = pd.concat(
            [known_prediction_rows, unknown_prediction_rows],
            ignore_index=False,
        ).sort_index(kind="stable").drop(
            columns=["_merge_gray_priority", "_merge_ok_priority"]
        )
        print("Resolved duplicate image codes by source/status priority.")

    # ------------------------------------------------------------------
    # Prediction columns to append to the combined laboratory data
    # ------------------------------------------------------------------
    desired_prediction_columns = ["SampleCode", *IMAGE_PREDICTION_FIELDS]
    ...
    # Main merge
    # ------------------------------------------------------------------
    # Pandas considers null join keys equal. Keep unidentifiable image rows
    # out of the merge, and preserve unidentifiable laboratory rows separately.
    # This also lets us retain strict one-to-one validation for real codes.
    known_lab = lab.loc[lab["SampleCode"].notna()].copy()
    unknown_lab = lab.loc[lab["SampleCode"].isna()].copy()
    known_predictions = predictions.loc[
        predictions["SampleCode"].notna(), prediction_columns
    ]

    known_lab["_merge_original_order"] = known_lab.index
    enriched = known_lab.merge(
        known_predictions,
        on="SampleCode",
        how="left",
        validate="one_to_one",
    )
    if not unknown_lab.empty:
        unknown_lab["_merge_original_order"] = unknown_lab.index
        for column in prediction_columns:
            if column != "SampleCode":
                unknown_lab[column] = pd.NA
        enriched = pd.concat([enriched, unknown_lab], ignore_index=True)
    enriched = (
        enriched.sort_values("_merge_original_order", kind="stable")
        .drop(columns="_merge_original_order")
        .reset_index(drop=True)
    )

    # The colour columns must now come from this run, not from any laboratory
    # workbook. Report a readiness count before subsequent model training.
    numeric_features = ["orgC_lab", "L", "a", "b"]
    if all(column in enriched.columns for column in numeric_features):
        complete_features = pd.DataFrame(
            {
                column: pd.to_numeric(enriched[column], errors="coerce")
                for column in numeric_features
            }
        ).notna().all(axis=1)
        usable_count = int(
            (enriched["SampleCode"].notna() & complete_features).sum()
        )
        print(
            "Usable laboratory/image rows with SampleCode, "
            f"orgC_lab, L, a and b: {usable_count} / {len(enriched)}"
        )
        ...
        [
            {
                ...
                "lab_input_rows": raw_lab_row_count,
                "lab_rows": len(lab),
    ...
    print(f"Input laboratory rows: {raw_lab_row_count}")
    ...
    print(
        ...
        f"{len(duplicate_lab_input_rows)}"
    )
    print(
    ...
    if not laboratory_sources.empty:
        print("\nInput laboratory rows by source file:")
        for source_file, rows in laboratory_sources.groupby(
            "_lab_source_file", dropna=False
        ):
            print(f"  {source_file}: {int(rows['row_count'].sum())}")
    ...
    if not duplicate_lab_input_rows.empty:
        ...
        print(
            duplicate_lab_input_rows[columns].to_string(
                index=False
            )
        )
    ...
    with pd.ExcelWriter(
        ...
        duplicate_lab_input_rows.to_excel(
            writer,
        ...
        # Counts refer to the original rows in each source file, not just
        # the winning source retained during duplicate resolution.
        if not laboratory_sources.empty:
    ...
    duplicate_lab_input_rows.to_csv(
